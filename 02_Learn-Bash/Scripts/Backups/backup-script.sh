...
if [ -z "$Backup_file" ]
then
	echo "[$(date +%F-%R)] ERROR. Please, Enter the Directory that you want to Backup" | tee -a "$LOG_FILE" | &> /dev/null
	exit 2
fi
...
then
	...
	if [ -f "$filename" ]
	then
		echo "[$(date +%F-%R)] ERROR. The file $filename is already exists" | tee -a "$LOG_FILE"
	else
		tar -czvf "$Dest/$filename" "$Backup_file" &> /dev/null

		echo 
		echo "[$(date +%F-%R)] INFO. Backup Completed successfuly, Backup file: $Dest/$filename" | tee -a "$LOG_FILE" 
	fi
fi
